...
printHelp() {
    printf "%s\n" "compile script for rogue_clone"
    printf "%s\n" "compiles rogue_clone and creates executable names 'rogue_clone'"
...
useGCC() {
    printf "%s\n" "using GCC"
    gcc @src_files -Wall -Wextra -O2 -Iheaders -finline-functions -Wswitch-enum -lncurses -std=gnu11 -orogue_clone
}
